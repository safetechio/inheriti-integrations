...
const styles = `<style>
  ...
  *{box-sizing:border-box}body{margin:0;min-height:100vh;display:grid;place-items:center;padding:24px}
  ...
  .status small{display:block;color:#475467;font-size:12px;margin-top:4px}
  .dot{width:12px;height:12px;flex:none;border-radius:50%;background:#2962ff;box-shadow:0 0 0 5px #e3f2fd;margin-top:4px}
  .device-panel{display:flex;align-items:center;gap:20px;background:#fafcff;border:1px solid #e3f2fd;border-radius:8px;padding:16px;margin:20px 0;color:#475467;font-size:14px}
  .device-panel img{width:92px;height:92px;object-fit:contain;flex:none}.device-panel .status{flex:1;background:#fff}
  .secret{margin:20px 0;padding:20px;background:#fafcff;border:1px solid #e3f2fd;border-radius:8px;white-space:pre-wrap;overflow-wrap:anywhere;font:16px/1.5 ui-monospace,SFMono-Regular,monospace;color:#101828}
  .file-card{display:flex;align-items:center;gap:16px;padding:20px;border:1px solid #e3f2fd;border-radius:8px;background:#fafcff;margin:20px 0}
  .file-icon{display:grid;place-items:center;width:52px;height:56px;flex:none;background:#e3f2fd;color:#1642ba;border-radius:8px;font-size:12px;font-weight:700}
  .file-card strong,.file-card small{display:block;overflow-wrap:anywhere}.file-card small{color:#475467;font-size:12px;margin-top:6px}
  footer{padding:16px 32px;border-top:1px solid #eaecf0;color:#475467;font-size:12px}
  @media(max-width:520px){body{padding:12px}header,.content{padding:20px}footer{padding:16px 20px}dl{grid-template-columns:1fr}dd{margin-bottom:6px}.device-panel{gap:8px}.device-panel img{width:64px;height:64px}}
</style>`;

export function brandPage(title: string, content: string, options: { eyebrow?: string; footer?: string; refresh?: boolean } = {}) {
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${title} · Inheriti Business</title>${options.refresh ? '<meta http-equiv="refresh" content="2">' : ''}${styles}</head><body><main><header><img src="${logo}" alt="Inheriti Business"></header><div class="content"><span class="eyebrow">${options.eyebrow ?? 'Local approval'}</span><h1>${title}</h1>${content}</div><footer>${options.footer ?? 'This request stays on your device. Keep this page open until it completes.'}</footer></main></body></html>`;
}
